...
Le migrazioni EF Core sono `LegacyBaseline` (schema CV preesistente), `AuthPlansOwnership` (Identity, sessioni, piani, subscription, usage e campi video/ownership) e `SessionActivity` (ultima attività delle sessioni, colonna nullable). L'API usa `Migrate`, non `EnsureCreated`. Per un database CV senza storico migrazioni verifica le colonne delle cinque tabelle esistenti, registra la baseline e applica le migrazioni additive; se trova uno schema diverso si ferma. Un advisory lock serializza la migrazione e il seed tra istanze API.

Il pannello **Monitoraggio amministrativo**, disponibile solo agli Admin come `GET /api/admin/dashboard`, si aggiorna ogni 30 secondi quando la pagina è visibile. Mostra utenti registrati, utenti attivi negli ultimi 5 minuti, sessioni valide, partite/video, spazio video registrato, utilizzo mensile e analisi per stato. La tabella elenca fino a 100 utenti ordinati per attività, con piano, ultima attività e numero di sessioni. Più dispositivi dello stesso utente contano come un solo utente attivo.

L'attività indica richieste autenticate, compreso il polling automatico: non certifica che una persona stia interagendo con la pagina. `LastSeenAt` viene aggiornato al login/refresh e al massimo una volta al minuto durante le richieste; le sessioni precedenti alla migrazione saranno rilevate alla prima richiesta. Una sessione valida può restare inattiva fino alla scadenza o al logout. Lo spazio video deriva dai metadati DB, non misura checkpoint/debug o l'intero disco; il pannello segnala i video legacy senza dimensione registrata. Le analisi per stato escludono quelle eliminate, mentre l'utilizzo mensile conserva i relativi addebiti e indica minuti richiesti, non elaborazione effettiva.
